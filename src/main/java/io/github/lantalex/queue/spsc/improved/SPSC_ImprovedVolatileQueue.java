...
public class SPSC_ImprovedVolatileQueue<E> implements SPSC_BoundedQueue<E> {
    ...
    public SPSC_ImprovedVolatileQueue(int capacity) {
    ...
    public boolean tryProduce(E value) {
        ...
        if (pNextIdx == consumerIdxCached.getPlain()) {
            consumerIdxCached.setPlain(consumerIdx.getVolatile());
    ...
    public boolean tryConsume(Consumer<E> consumer) {
        ...
        if (cIdx == producerIdxCached.getPlain()) {
            producerIdxCached.setPlain(producerIdx.getVolatile());
